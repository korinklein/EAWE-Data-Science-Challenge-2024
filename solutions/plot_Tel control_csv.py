...
def density_filter(rotorspeed, Tel, Tmech, bins, percentile_threshold=2):    
    ...
    Tmech_filtered = Tmech[densitymask]

    return rotorspeed_filtered, Tel_filtered, Tmech_filtered



# find most often occuring pitch angles

def pitchangles_to_be_considered(pitchangle):

    # consider only pitch angles that are set
    gradient = np.diff(pitchangle)
    zero_slope_indices = np.where(gradient == 0)[0]
    zero_slope_values = pitchangle[zero_slope_indices + 1]
    
    # count frequency of pitch angles
    pitch_counts = Counter(zero_slope_values)

    # sort frequency
    sorted_counts = sorted(pitch_counts.items(), key=lambda x: x[1], reverse=True)

    return sorted_counts #pitchconsidered
...
def calculate_bins(rotorspeed, torque, pitchangle, bins=30, min_count=20):

    if rotorspeed.size == 0 or torque.size == 0:
        print(f"Warning: No valid bins for pitch angle {pitchangle}. Skipping.")
...
# create plot
def create_plot(start_date, end_date, rotorspeed_all, torque_all, rotorspeed_pitch, torque_pitch, rotorspeed_pitch_filtered, 
                torque_pitch_filtered, pitchangle, pitch_freq, bin_centers, bin_means, ax_plot):

    # Set plot marker size
    plt.rcParams['lines.markersize'] = 2

    ax_plot.set_title(f"Torque vs Rotor speed (pitch = {pitchangle} ({pitch_freq} points); date range: {start_date} - {end_date})")
    ax_plot.plot(rotorspeed_all, torque_all, '.', label='Total')
    ax_plot.plot(rotorspeed_pitch, torque_pitch, '.', label='Considered pitch only', color='tab:cyan')
    ax_plot.plot(rotorspeed_pitch_filtered, torque_pitch_filtered, '.', label='Considered pitch density filtered', color='pink')
...
def plot_control_parameters(start_date, end_date, time, GenTorqSP, current, voltage, rotorspeed, pitchangle, Tmech):
    ...
    pitchangle_mask = pitchangle[NaNmask]
    Tmech = Tmech[NaNmask]
    ...
    pitchangle_mask = pitchangle_mask[Telmask]
    Tmech = Tmech[Telmask]
    Tel = Tel[Telmask]


    # consider most often occuring pitchangles
    pitchconsidered = pitchangles_to_be_considered(pitchangle_mask)


    # apply function on angles individually

    for pitch, pitch_freq in pitchconsidered:

        
        # pitch angle mask
        pitchmask = (pitchangle_mask == pitch)
        rotorspeed_pitch = rotorspeed[pitchmask]
        Tel_pitch = Tel[pitchmask]
        Tmech_pitch = Tmech[pitchmask]

        if rotorspeed_pitch.size == 0 or Tmech_pitch.size == 0:
            print(f"Warning: No valid data points for pitch angle {pitch}. Skipping.")
            continue


        # apply density filter because pitch filter seems to be not enough
        rotorspeed_pitch_filtered, Tel_pitch_filtered, Tmech_pitch_filtered = density_filter(rotorspeed_pitch, Tel_pitch, Tmech_pitch, bins=50)

        
        # calculate bins
        bin_means, bin_centers, bin_slopes = calculate_bins(rotorspeed_pitch_filtered, Tel_pitch_filtered, pitch)
        if bin_means.size == 0 or bin_centers.size == 0 or bin_slopes.size == 0:
            print(f"Warning: No valid data points for pitch angle {pitch}. Skipping.")
            continue
        ...
        # plot
        create_plot(start_date=start_date, end_date=end_date, rotorspeed_all=rotorspeed, torque_all=Tel, rotorspeed_pitch=rotorspeed_pitch, 
                                        torque_pitch=Tel_pitch, rotorspeed_pitch_filtered=rotorspeed_pitch_filtered, torque_pitch_filtered=Tel_pitch_filtered, 
                                        pitchangle=pitch, pitch_freq=pitch_freq, bin_centers=bin_centers, bin_means=bin_means, ax_plot=ax_plot)
        ...
        if (mean_slope_first > mean_slope_second > 0) and not (np.abs(mean_slope_first) < 10) and (len(regions) > 1):
            ...
            Tmechfilter_rotorspeed1 = (rotorspeed_intersection152 - 0.5 < rotorspeed_pitch_filtered) & (rotorspeed_pitch_filtered < 0.5 + rotorspeed_intersection152)
            Tmech_rotorspeed1 = Tmech_pitch_filtered[Tmechfilter_rotorspeed1]
            Tmech_average_rotorspeed1 = np.mean(Tmech_rotorspeed1)

            Tmechfilter_rotorspeed2 = (rotorspeed_center_second - 0.5 < rotorspeed_pitch_filtered) & (rotorspeed_pitch_filtered < 0.5 + rotorspeed_center_second)
            Tmech_rotorspeed2 = Tmech_pitch_filtered[Tmechfilter_rotorspeed2]
            ...
            control_parameters["VS_Rgn2K"] = (mean_slope_second/0.85*4*np.pi**2/60, "W/(rad/s)^2")
        ...
        if (0 < mean_slope_first < mean_slope_second) and not (np.abs(mean_slope_first) < 10) and (len(regions) > 1):
            ...
            # calculate slope of Tmech for K
            Tmechfilter_rotorspeed1 = (rotorspeed_center_first - 0.5 < rotorspeed_pitch_filtered) & (rotorspeed_pitch_filtered < 0.5 + rotorspeed_center_first)
            Tmech_rotorspeed1 = Tmech_pitch_filtered[Tmechfilter_rotorspeed1]
            Tmech_average_rotorspeed1 = np.mean(Tmech_rotorspeed1)

            Tmechfilter_rotorspeed2 = (rotorspeed_intersection225 - 0.5 < rotorspeed_pitch_filtered) & (rotorspeed_pitch_filtered < 0.5 + rotorspeed_intersection225)
            Tmech_rotorspeed2 = Tmech_pitch_filtered[Tmechfilter_rotorspeed2]
            Tmech_average_rotorspeed2 = np.mean(Tmech_rotorspeed2)
        ...
        for (rotorspeed_center, Tel_center, mean_slope, _, _) in regions:
            ...
            Tel_values = Tel_center + mean_slope*(rotorspeed_pitch_filtered - rotorspeed_center)

            # plot
            ax_plot.plot(rotorspeed_pitch_filtered, Tel_values, '-', linewidth=3)
    ...
    # instances of control changes
    instances_of_control_change(time=time, rotorspeed=rotorspeed, pitchangle=pitchangle, rotorspeed_rated=rotorspeed_intersection253)
...
def process_csv_files():
    ...
    all_pitchangle_array = np.array(all_pitchangle)
    ...
    plot_control_parameters(start_date=start_date, end_date=end_date, time=all_time_array, GenTorqSP=all_GenTorqSP_array, 
                            current=all_current_array, voltage=all_voltage_array, rotorspeed=all_rotorspeed_array, 
                            pitchangle=all_pitchangle_array, Tmech=all_Tmech_array)
...
def process_and_collect(data):
    ...
    all_pitchangle.extend(pitchangle)
